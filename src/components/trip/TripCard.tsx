...
import { TripRouteCardProps } from "./Trip.types";

export default function TripCard({ stops }: TripRouteCardProps) {
    return (
        <View className="flex-1 bg-white rounded-2xl p-5 shadow-md" style={{ maxHeight: '70%' }}>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
                ...
                const label =
                    stop.label ?? (isLast ? "Destination" : `Stop ${index}`);
                ...
                return (
                    ...
                    {!isLast && (
                        ...
                        {stops[index + 1].distanceFromPrev != null && (
                            <Text className="text-xs text-gray-400 ml-3">
                            {stops[index + 1].distanceFromPrev} mi ·{" "}
                            {stops[index + 1].durationFromPrev} min
                            </Text>
                        )}
                        </View>
                    )}
